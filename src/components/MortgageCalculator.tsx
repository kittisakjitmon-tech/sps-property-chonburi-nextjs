...
export default function MortgageCalculator({ price, directInstallment = false }: MortgageCalculatorProps) {
  const [loanType, setLoanType] = useState<'bank' | 'direct'>(directInstallment ? 'direct' : 'bank');
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">
            อัตราดอกเบี้ย (% ต่อปี) — {loanType === 'direct' ? 'ผ่อนตรง' : 'กู้ธนาคาร'}
          </label>
          <input
            type="number"
            step="0.1"
