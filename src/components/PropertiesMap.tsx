...
export default function PropertiesMap({ properties, className = '' }: PropertiesMapProps) {
  ...
  const propertiesWithCoords = properties.filter(
    (p) => p.lat != null && p.lng != null && !isNaN(p.lat as number) && !isNaN(p.lng as number)
  );
  ...
  useEffect(() => {
    ...
    loadLongdoMap()
      ...
      .catch((error) => {
        ...
        const msg = error?.message?.includes('NEXT_PUBLIC_LONGDO_MAP_KEY') || !process.env.NEXT_PUBLIC_LONGDO_MAP_KEY
          ? 'ไม่สามารถโหลดแผนที่ได้ (กรุณาตั้งค่า NEXT_PUBLIC_LONGDO_MAP_KEY ใน .env)'
  ...
  useEffect(() => {
    ...
    // ถ้าไม่มีข้อมูลพิกัด ให้ล้างแผนที่แล้วจบการทำงาน
    ...
    const unbindOverlayClick = map.Event?.bind?.('overlayClick', (overlay: any) => {
      try {
        ...
        // ป้องกัน error จาก Longdo ภายใน
      }
    });
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            const closeBtn = (node as Element).querySelector?.('[data-popup-close]');
            if (closeBtn) {
              closeBtn.addEventListener('click', closePopup);
            }
            (node as Element).querySelectorAll?.('[data-popup-close]').forEach((btn) => {
