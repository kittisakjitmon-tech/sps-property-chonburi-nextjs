...
let longdoLoadingPromise: Promise<any> | null = null;

declare global {
  interface Window {
    longdo: any;
    spsCloseMapPopup?: () => void;
  }
}
...
export function loadLongdoMap(): Promise<any> {
  ...
  if (!apiKey) {
    longdoLoadingPromise = Promise.reject(new Error('Missing NEXT_PUBLIC_LONGDO_MAP_KEY'));
    return longdoLoadingPromise;
  }
